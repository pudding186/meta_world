...
public:
    ...
    using Row = SFieldList<
        ...
        mt_billlog::award_yb,
        mt_billlog::time,
        mt_billlog::user_name,
        mt_billlog::bill_desc
    >;
    using PriKey = SFieldList<
        mt_billlog::bill_id
    >;
    mt_billlog(HMYSQLCONNECTION connection = nullptr)
        :IDBTable(connection), m_cache(*this){}

    static const std::string& TableName(void)
    {
        static std::string table_name = u8"mt_billlog";
        return table_name;
    }
    ...
    const std::string& GetTableName(void) const override { return mt_billlog::TableName(); }
    const std::string& GetTableExtra(void) const override { return mt_billlog::TableExtra(); }
    const std::string& GetTableEngine(void) const override { return mt_billlog::TableEngine(); }
    const std::string& GetTableCharset(void) const override { return mt_billlog::TableCharset(); }
    const std::string& GetTableRowFormat(void) const override { return mt_billlog::TableRowFormat(); }
    ...
    {
        return PriKey(
        row.Field<mt_billlog::bill_id>().GetData()
        );
    }
    ...
    inline DBTableCache<mt_billlog>& Cache(void) { return m_cache; }
    DBTableCache<mt_billlog> m_cache;
};
