...
size_t _escape_to(const char* src, size_t n, char* dst)
{
    char* p = dst;
    char *s = (char*)src, *e = (char*)src + n;
    while (s < e)
...
size_t _escape_from(const char* src, size_t n, char* dst)
{
    char* p = dst;
    char *s = (char*)src, *e = (char*)src + n;
    while (s < e)
...
void _json_struct_add_node(json_struct* json_struct_ptr, json_node* json_node_ptr)
{
...
bool _step_number(const char** ptr)
{
...
void _step_string(const char** ptr)
{
    for (; ;)
    {
        switch (**ptr)
...
bool _isdouble(const char* pDigit, size_t nSize)
{
    char* s = (char*)pDigit;
...
void _json_log_error(const char* json_str_begin, const char* current_ptr, char* err_msg, size_t err_msg_size)
{
...
size_t json_to_string(json_struct* json_struct_ptr, char* json_str_ptr, size_t json_str_size)
{
    char tmp_buf[64];
    size_t tmp_length = 0;
    ...
    while (node_ptr)
    {
        if (node_ptr->key)
        {
            if (end_pos - current_pos > 3 + (int)node_ptr->key_length * 2)
            {
                *current_pos = '\"';
                current_pos++;
                current_pos += _escape_to(node_ptr->key, node_ptr->key_length, current_pos);
        ...
        case json_string:
        {
            if (end_pos - current_pos > 2 + (int)node_ptr->str_length * 2)
            {
                *current_pos = '\"';
                ++current_pos;
                current_pos += _escape_to(node_ptr->value.str, node_ptr->str_length, current_pos);
                *current_pos = '\"';
                current_pos += 1;
            }
            else
...
void json_to_string_ex(json_struct* json_struct_ptr, json_string_append f, void* ud)
{
    char tmp_buf[64];
    size_t tmp_length = 0;
    struct st_json_node* node_ptr = json_struct_ptr->head;
    ...
    while (node_ptr)
    {
        ...
        {
            (*f)(ud, "\"", 1);
            _escape_to_json_append(node_ptr->key, node_ptr->key_length, f, ud);
            (*f)(ud, "\":", 2);
        }
        ...
        {
            (*f)(ud, "\"", 1);
            _escape_to_json_append(node_ptr->value.str, node_ptr->str_length, f, ud);
            (*f)(ud, "\"", 1);
...
json_struct* create_json(enum e_json_value_type type)
{
    switch (type)
    {
    case json_object:
...
json_struct* json_add_struct(json_struct* json_struct_ptr,
...
bool _step_key_value(const char** ptr, struct st_json_struct* json_struct_ptr, const char** key_ptr, size_t* key_length)
{
    bool is_double;
...
bool _step_value(const char** ptr, struct st_json_struct* json_struct_ptr)
{
    bool is_double;
    const char* value_ptr = 0;
    size_t value_length = 0;
...
json_struct* string_to_json(const char* str, char* err_info, size_t err_info_len)
{
    ...
    for (;;)
    {
        ...
        case '[':
        {
            ...
            if (*current_ptr != ']')
            {
                if (!_step_value(&current_ptr, current_json_struct))
                {
                    if (err_info && err_info_len)
                    {
                        _json_log_error(str, current_ptr, err_info, err_info_len);
                    }
        ...
        case '{':
        {
            ...
            if (*current_ptr != '}')
            {
                if (!_step_key_value(&current_ptr, current_json_struct, &key_ptr, &key_length))
                {
                    if (err_info && err_info_len)
                    {
                        _json_log_error(str, current_ptr, err_info, err_info_len);
...
json_node* json_struct_first_node(json_struct* json_struct)
{
    return json_struct->head;
}

json_node* json_struct_next_node(json_node* json_node)
{
    return json_node->next;
}

const char* json_key(json_node* json_node)
{
    return json_node->key;
}

bool json_value_integer(json_node* json_node, long long* value)
{
...
bool json_value_float(json_node* json_node, double* value)
{
...
bool json_value_string(json_node* json_node, const char** value, size_t* length)
{
...
bool json_value_true(json_node* json_node)
{
...
bool json_value_false(json_node* json_node)
{
...
bool json_value_null(json_node* json_node)
{
...
json_struct* json_value_struct(json_node* json_node)
{
...
json_value_type json_node_type(json_node* json_node_)
{
    return json_node_->type;
}

json_value_type json_struct_type(json_struct* json_struct_)
{
    return json_struct_->type;
}
